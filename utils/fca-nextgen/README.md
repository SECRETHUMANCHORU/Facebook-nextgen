...
# Nextgen Facebook Chat API
# Modified Choru Tiktokers, Shiki Machina, Joland Manzano
# Ai by Choru Tiktokers

```javascript
let result;
        result = await api.openai("hi");
        console.log(result)
```

# api.getUserInfo update features
# userInfo.highResProfilePic
# userInfo.uid
# Modified Choru TikTokers
```javascript
let targetID = `${event.body.split(" ") || event.senderID}`; 
...
`;
const imgPath = path.join(__dirname, '../cache/avtpfp.jpg');

            const sendMsgWithImage = () => {
                api.sendMessage({
                    body: formattedInfo,
            ...
            request(userInfo.highResProfilePic).pipe(fs.createWriteStream(imgPath)).on("close", sendMsgWithImage);
            
```

<a href="https://www.npmjs.com/package/fca-nextgen"><img alt="npm version" src="https://img.shields.io/npm/v/fca-nextgen.svg?style=flat-square"></a>
...
<a href="https://www.npmjs.com/package/fca-nextgen"><img src="https://img.shields.io/npm/dm/fca-nextgen.svg?style=flat-square" alt="npm downloads"></a>
...
## Install
If you just want to use fca-nextgen, you should use this command:
```bash
npm install fca-nextgen
```
It will download `fca-nextgen` from NPM repositories
...
## Example Usage
```javascript
const login = require("fca-nextgen");
